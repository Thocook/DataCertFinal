...
app.layout = html.Div(children=[html.H1('SpaceX Records Dashboard',
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        fig = px.pie(filtered_df, values='class', names='class', title=f'Success and Failure Counts for {entered_site}')
        return fig
...
def update_scatter_chart(entered_site, selected_payload):
    if entered_site == 'ALL':
        ...
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', color='Booster Version Category',
                         title='Payload and Outcome Scatter Chart for All Sites',
                         labels={'class': 'Launch Outcome', 'Payload Mass (kg)': 'Payload Mass (kg)'},
                         category_orders={'class': ['Failure', 'Success']})
        return fig
    else:
        # If a specific launch site is selected, filter the spacex_df and render a scatter chart for the selected site
        filtered_df = spacex_df[(spacex_df['Launch Site'] == entered_site) &
                                (spacex_df['Payload Mass (kg)'] >= selected_payload[0]) & 
                                (spacex_df['Payload Mass (kg)'] <= selected_payload[1])]
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', color='Booster Version Category',
                         title=f'Payload and Outcome Scatter Chart for {entered_site}',
                         labels={'class': 'Launch Outcome', 'Payload Mass (kg)': 'Payload Mass (kg)'},
                         category_orders={'class': ['Failure', 'Success']})
        return fig
